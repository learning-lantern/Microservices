...
public static class ResponseFactory
{
    ...
    public static Response<T> Fail<T>(T? data = default) => new(false, data, default);
    ...
    public static TResponse? CreateFailObject<TResponse>(IEnumerable<ErrorBase> failures) where TResponse : Response
    {
        var responseType = typeof(TResponse);

        if (!responseType.IsGenericType)
        {
            return Activator.CreateInstance(responseType, false, failures) as TResponse;
        }
...
{
    public Response(bool succeeded, T? data, IEnumerable<ErrorBase>? errors) : base(succeeded, errors)
    {
        Data = data;
    }
